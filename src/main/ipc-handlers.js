...
function getReportColumns(type) {
  switch (type) {
    ...
    case 'issueReport':
      return [
        { key: 'issue_id', label: 'Issue ID', width: 50 },
        { key: 'issue_date', label: 'Date', width: 45, format: (v) => v ? new Date(v).toLocaleDateString('en-GB') : '' },
        { key: 'issue_type', label: 'Type', width: 35 },
        { key: 'recipient_name', label: 'Recipient', width: 60 },
        { key: 'item_name', label: 'Issued Material', width: 75 },
        { key: 'item_code', label: 'Mat Code', width: 45 },
        { key: 'target_products_summary', label: 'Target Finished Item(s)', width: 85, format: (v, r) => r.target_products_summary || '-' },
        { key: 'style_name', label: 'Style', width: 50, format: (v) => v || '-' },
        { key: 'purchase_no', label: 'Purchase No', width: 50, format: (v) => v || '-' },
        { key: 'order_number', label: 'Order No', width: 50, format: (v) => v || '-' },
        { key: 'size', label: 'Size', width: 35, format: (v) => v || '-' },
        { key: 'color', label: 'Color', width: 35, format: (v) => v || '-' },
        { key: 'buyer_name', label: 'Buyer', width: 55, format: (v) => v || '-' },
        { key: 'quantity', label: 'Issued', width: 40, align: 'right' },
        { key: 'returned_quantity', label: 'Returned', width: 35, align: 'right' },
        { key: 'damage_quantity', label: 'Damaged', width: 40, align: 'right' },
        { key: 'rejected_quantity', label: 'Rejected', width: 40, align: 'right' },
        { key: 'outstanding', label: 'Outstanding', width: 40, align: 'right' },
        { key: 'unit', label: 'Unit', width: 35, format: (v) => v || '-' },
