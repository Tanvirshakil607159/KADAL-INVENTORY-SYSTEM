      ...
      try {
        ...
        if (row.category) {
          ...
          if (existing) {
            ...
            const r = CategoriesRepo.create({ name: row.category });
        ...
        let supplierId = null;
        if (row.supplier) {
          const supps = SuppliersRepo.getAll();
          const existing = supps.find(s => s.name.toLowerCase() === row.supplier.toLowerCase());
          if (existing) {
            supplierId = existing.id;
          } else {
            const r = SuppliersRepo.create({ name: row.supplier });
