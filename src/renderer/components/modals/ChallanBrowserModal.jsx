...
export default function ChallanBrowserModal({ data }) {
  ...
  const addItem = async (item) => {
    ...
    if (challanItems.some(i => i.itemId === item.id)) return addToast('warning', 'Already added');
  ...
  return (
    <div className={`modal-overlay ${isMinimized ? 'minimized-mode' : ''}`}>
      <div className={`modal modal-lg ${isMinimized ? 'minimized' : ''}`} onClick={e => e.stopPropagation()} style={{ maxWidth: 1300, width: '95vw' }}>
        <div className="modal-header">
          <h3 className="modal-title">Browse Inventory — Select Items for Challan</h3>
        ...
        <div className="modal-body">
          ...
          <div className="search-bar mb-3" style={{ maxWidth: '100%' }}>
            <Search size={16} />
            <input className="form-input" placeholder="Search by name, code, style, order, or purchase..." value={filters.search} onChange={e => setFilters({ ...filters, search: e.target.value })} style={{ fontSize: 14 }} />
          </div>
          {/* Filter row */}
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
            ...
            <SearchableSelect className="form-select" value={filters.order} onValueChange={value => setFilters({ ...filters, order: value })} style={{ width: 130, fontSize: 12 }}>
              <option value="">All Orders</option>
              {(distinctValues.orders || []).map((v, i) => <option key={i} value={v}>{v}</option>)}
            </SearchableSelect>
            <SearchableSelect className="form-select" value={filters.purchase} onValueChange={value => setFilters({ ...filters, purchase: value })} style={{ width: 130, fontSize: 12 }}>
                ...
                {filteredItems.map(item => {
                  ...
                  return (
                    <tr key={item.id} style={{ opacity: (added || isLocked) ? 0.5 : 1 }}>
                      ...
                      <td>
                        {isLocked ? (
                          <span className="badge badge-warning" style={{ fontSize: 10, whiteSpace: 'nowrap' }}>Pending Approval</span>
                        ) : (
